...
class T800FlatWoStateEstimationEnvCfgV15Scale(T800FlatWoStateEstimationEnvCfgV14Scale):
    """V14-scale with horizon-corrected states and duration-proportional sampling."""

    def __post_init__(self):
        super().__post_init__()
        motion = self.commands.motion
        # One 50 Hz bin measures local start feasibility. Reaching the horizon
        # records the label without ending the episode; later failures are
        ...
        # Keep equal base exposure per 50-frame bin. A motion with four times
        # as many bins should receive four times the total mass (approximately
        # proportional to duration, up to its final partial bin).
        motion.curriculum_motion_length_exponent = 1.0
